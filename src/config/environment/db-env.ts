...
export function resolveDialectCredentials(
  ...
  switch (dialect) {
    ...
    default: {
      ...
      throw new Error(
        `Error de configuración: DB_DIALECT inválido (${String(unsupported)}). Use mysql, postgres, mssql u oracle.`,
      );
    }
  }
}
...
export function assertActiveDialectCredentials(config: DatabaseConfig): void {
  ...
  if (missing.length > 0) {
    throw new Error(
      `Error de configuración: falta la variable ${missing.join(', ')}. Completa el bloque del motor activo (${config.dialect}) en .env.`,
    );
  }
}
